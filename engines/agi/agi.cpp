...
AgiEngine::AgiEngine(OSystem *syst, const AGIGameDescription *gameDesc) : AgiBase(syst, gameDesc) {
	...
	// On the Playdate the AGI mouse stays enabled - mouse/menu games like
	// Manhunter need it - but the backend hides the software cursor unless
	// the player is in pointer mode (see PlaydateGraphicsManager), so
	// keyboard/parser games show no stray cursor.
